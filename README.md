...
- **项目类型**：创建时可选择原创小说或授权改写项目；改写项目用于导入已获授权的 TXT 参考小说、生成结构化参考分析、管理改写意见并确认改编总方案
...
- **授权参考改写（阶段 1-4）**：导入 TXT 参考小说，分批分析全书结构，管理改写意见，确认改编总方案后逐章完全重写，并执行三项检查；重点章节可审计地启用原文全文参考和更严格相似度阈值
...
创建项目时选择「授权改写」后，左侧导航会切换为改写专属入口。当前阶段支持：

1. **导入 TXT 参考小说**：浏览器读取 `.txt` 后提交正文，后端自动按「第 N 章 / Chapter N」等常见标题拆章，并把原章正文保存为独立 TXT。
2. **审核章节边界**：可编辑章节标题和正文，也可插入章节完成手工拆分；保存后旧分析会清空，避免使用过期结构。
3. **分批参考分析**：AI 按章节/分块生成每章摘要、关键事件、章节功能、情绪曲线、伏笔回收点，再合并出全书梗概、核心设定和结构说明。
4. **设定候选导入**：空项目会自动填充抽取出的设定；已有设定时只展示候选，需手动确认导入，不会静默覆盖。
5. **改写意见管理**：可添加全局、单章、章节范围、角色长期变化、设定 / 金手指规则、关系线、结局路线和禁止项意见。
6. **改编总方案**：AI 根据参考分析和改写意见生成可编辑方案，包含意见影响地图、角色/设定/关系变化表、多对多章节映射和逐章新稿计划；确认时会校验每个原文章节都被覆盖，然后生成新稿章节骨架。
7. **逐章完全重写**：在「写作」页按新稿章节骨架生成正文；普通章节默认只把结构化参考分析喂给 AI，不把原文全文作为逐句参照。
8. **三项检查闭环**：每章生成后自动检查改写意见符合度、结构保真和贴近原文风险；贴近风险结合确定性相似度算法（字符 n-gram、句子重合、连续长片段）与 AI 判断，不通过会自动重写并保存检查结果。
9. **中途新增意见（最小版）**：方案确认后新增或修改意见不会静默覆盖已写章节；已写章节会标记需复核 / 需重写，未写章节计划会追加新约束。
10. **重点章节全文参考**：可在「改编方案」页为单章开启原文全文参考并填写理由；生成该章时会注入映射原章全文，并启用更严格的贴近度阈值。
11. **改写专项报告**：完稿后可在全书优化面板生成意见落实、结构保真、相似度风险、角色/设定变化一致性四份报告；相似度报告会汇总高风险章节和开启原文全文参考的章节。
12. **TXT / Markdown 导出**：写作页可导出完整新稿 TXT 或 Markdown；每章确认/修订时也会同步保存为项目目录内的 Markdown 文件。
...
└── storys/
    └── <项目名>/
        ...
        ├── reference.json   # 改写项目：参考书元数据与章节列表
        ├── reference_analysis.json # 改写项目：参考书结构化分析
        ├── rewrite_requests.json   # 改写项目：用户改写意见
        ├── rewrite_plan.json       # 改写项目：改编总方案、章节映射与逐章检查结果
        ├── postprocess.json        # 全书优化与改写专项报告
        ├── reference/       # 改写项目：原文章节 TXT
        ├── sessions/        # 助理聊天记录
